...
A second authentication mechanism exists now for deployments that want it: mutual TLS, where the agent proves possession of a private key during the handshake rather than sending a secret. That does not make a stolen credential impossible, it changes what has to be stolen, from a string that can be copied out of a log to a private key. The binding is revocable the same way, see invariant 39.
...
**Detect:** Partial now, where before this pass it was none at all. When `NIA_GATEWAY_DOWNSTREAM_URL` is set, `cmd/gateway`'s `HTTPForwarder` actually forwards the call and `inspectAndAuditDownstream` looks at what came back for three specific things: the downstream's own HTTP status (5xx audited as `gateway.downstream_error`, 4xx as `gateway.downstream_rejected`), a top-level JSON `"error"` field in the body even on a 200 (`gateway.downstream_reported_error`), and a response that hit the 10 MiB read cap (`gateway.downstream_response_truncated`, worth a look on its own, a tool suddenly returning far more data than usual is a signal regardless of whether anything else about the call looked wrong). A fourth check looks at the response's structure rather than its status: `inspectResponseFields` walks the JSON body, matches field names and dotted paths against the operator's declared `internal/sensitivity` rules, and splits what it finds by whether the agent actually holds a `policy.GrantForData` for that field. A granted sensitive field is recorded (`gateway.downstream_sensitive_fields`), an ungranted one is a finding (`gateway.downstream_ungranted_fields`) and is scored into the same risk total the request side feeds, so an agent pulling fields nobody granted accumulates toward containment rather than only producing a log line.

That closes the specific case the request-side check cannot: an agent legitimately granted `database.query` on a table it is entitled to can still receive a column nobody granted, because the downstream decides what goes in the response. It is still not general content inspection. It matches declared names, so it will not recognize a social security number by its shape, and it says nothing about a field nobody declared, which makes it exactly as complete as the operator's rules are and explainable in the same way, this fired because you declared `customers.ssn` sensitive. A tool that returns plausible-looking poisoned output under ordinary field names still raises none of these signals.

**Contain:** Partial, and worth being precise about what kind. Nothing stops a compromised tool from returning bad data to an agent that then acts on it: by the time the response is inspected the downstream has already produced it and it is on its way back, so refusing at that point would be theatre. What a response-side finding does is accumulate risk, which can cross the revoke or kill thresholds and cut the agent off from making the next call. Actually preventing the data from arriving means declaring the resource up front so the request-side data-grant check refuses the call, which works today, or filtering responses field by field, which would mean NIA deciding what an agent may see rather than what it may call, a much larger design than this. What NIA can and does contain is access to the tool going forward: the same credential revoke and kill paths used everywhere else apply here too, an operator who identifies a compromised tool can deny it by killing or restricting the agents authorized to call it, not by anything specific to this threat.
...
**Not covered:** Value-level content inspection, so a sensitive value under an undeclared field name passes unnoticed; semantic detection of a subtly poisoned but well-formed answer; response filtering, see Contain above; and anything about a downstream when `NIA_GATEWAY_DOWNSTREAM_URL` isn't set, the default, in which case this threat is exactly as uncovered as it was before. Field inspection also needs `NIA_SENSITIVITY_RULES_PATH` configured, since with no rules nothing classifies above public and the walk finds nothing.
...
**Prevent:** Real now, and this is the half that was missing. `internal/ratelimit` is a token bucket applied twice on different keys. Keyed by client address it wraps both binaries' whole mux, in front of authentication, which is the case that matters most: an uncredentialed flood costs a credential or token lookup per request and is never audited, there is no resolved identity to attribute an event to. Keyed by agent ref it runs inside `handleToolCall` immediately after identity resolution and before the catalog lookup, the policy check, argument inspection, scoring and forwarding, so a refused call costs almost nothing. The two are not redundant, one agent can call from many addresses and one address can present many agents' credentials. Both default to on (50/s per client, 20/s per agent, per replica), because a limiter that only protects deployments whose operator remembered a variable protects almost nobody; `0` disables either explicitly. Alongside it, both servers now cap request bodies at 1 MiB and set `ReadTimeout`, `WriteTimeout`, `IdleTimeout` and `MaxHeaderBytes`, where only `ReadHeaderTimeout` was set before, so a client could send valid headers and then hold the connection open or trickle a body indefinitely.
...
**Not covered:** The limiter is per replica and in process, deliberately: a shared one means a database round trip on the hot path to defend against load, which is the wrong trade in exactly the situation it is for. With N replicas behind a load balancer the effective ceiling is N times the configured rate, so it is a blast shield, not a quota system. `X-Forwarded-For` is ignored unless `NIA_RATE_LIMIT_TRUST_FORWARDED_FOR=1`, because on a directly reachable process trusting it lets anyone mint unlimited buckets by varying a header; a deployment behind a proxy that does not set it is limiting the proxy rather than the client. Detection, separately, still needs the two rate variables set, and those numbers are a deployment's own to choose, there is no default that would be honest. The window is a flat count, not a baseline: this notices "more than N in the window," not "unusual for this agent specifically," which is the harder thing CENTIPEDE's own baselining is meant to bring. Sequence detection is one step deep, the immediately preceding tool, not a model of an agent's whole call graph. And a repeat of the same tool is deliberately not a novel transition, so a tight loop on one tool is caught by `call_rate` or not at all.
