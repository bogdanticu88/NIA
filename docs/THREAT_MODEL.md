...
**Not covered:** Actual authentication of a human operator or service calling `cmd/api` itself, that surface is still unauthenticated, `operator` fields throughout are client-supplied free text, not verified against anything. Hardening the control plane is separate, open work, tracked but not part of this fix.
