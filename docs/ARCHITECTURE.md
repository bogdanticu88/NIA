...
`deployments/docker-compose.yml` now wires all four services (`nia-api`, `nia-gateway`, `tessera`, `openfga`, plus `postgres`) together, with Dockerfiles for `cmd/api`, `cmd/gateway` (this repo), and `Tessera.Service` (the Tessera repo). None of that has run under actual Docker in this sandbox, there is no Docker daemon here, so `docker build`/`docker compose up` for all four services together, and Tessera against real OpenFGA specifically (every verification so far used Tessera's in-memory store), are both still open, see the comment at the top of that compose file and the Status section of the README for exactly what that needs.
