...
This intentionally does not carry a blast-radius snapshot from the identity graph yet, see the package's own doc comment for why: the graph isn't populated automatically from what's already known elsewhere, so a snapshot taken at containment time would be empty for most agents today, worse than not showing one. That's phase 4, not faked here.
