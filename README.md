...
# add the agent and a tool to the identity graph, and grant one access to the other
go run ./cmd/niactl graph add-node -id agent:billing-reconciler -kind agent
go run ./cmd/niactl graph add-node -id tool:invoice-lookup -kind tool
go run ./cmd/niactl graph add-edge -from agent:billing-reconciler -to tool:invoice-lookup -kind grants
