...
- `internal/audit.PostgresSink`: a real, shared `Store` backing the trail, `database/sql` plus `lib/pq`, schema created on first connect (`CREATE TABLE IF NOT EXISTS`, see `postgres_sink.go`, this table's shape has never changed and doesn't warrant a migration framework yet). `audit.FromEnv` picks it over `InMemorySink` when `NIA_AUDIT_DATABASE_URL` is set, same pattern as `policy.FromEnv`, and `deployments/docker-compose.yml` now points both NIA services at the stack's own `postgres` service so they share one trail instead of two. **Not yet run**, unlike everything above: this environment couldn't resolve the new `lib/pq` dependency (no network access to the Go module proxy), so it's unit-test-written but not `go build`-confirmed, let alone run against a live database. Before trusting this: `go mod tidy` (adds `lib/pq` to `go.mod`/`go.sum`), then `go build ./... && go vet ./... && go test ./... -race`, then bring up just `postgres` (`docker compose -f deployments/docker-compose.yml up -d postgres`) and run the opt-in live test against it, `NIA_AUDIT_TEST_DATABASE_URL="postgres://nia:nia@localhost:5433/nia?sslmode=disable" go test ./internal/audit/... -run TestPostgresSink_Live -v`, and finally the full compose stack to confirm `nia-api` and `nia-gateway` actually see each other's audit events once they're both pointed at it.
